    ...
    <div id="container" class="w-screen 2xl:w-[40%] bg-[#fbfbfb] rounded-lg relative mr-5 ml-5 justify-center z-20 p-right-5 p-left-5">
        ...
        <div class="flex justify-center">
            ...
            {#each grid_info as row}
                {#each row as seat}
                    {#if seat.type === 'seat'}
                        <button aria-label="none" onclick={() => toggleSeat(`${seat.id}`)} class="row-start-{seat.x} col-start-{seat.y} 
                        rounded-full flex justify-center items-center hover:opacity-60 active:scale-110"><svg id="{seat.id}" class="hover:fill-amber-200∫" 
