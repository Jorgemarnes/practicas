...
<h1 class="flex justify-center align-center m-[20px] font-bold text-[25px] sm:text-[30px]">Sesiones:</h1>

<div class="m-[10px] justify-evenly align-center 
flex-wrap grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 
gap-4 bg-gray-100 p-[20px] rounded-lg">
    {#each data.activities as activity}
        <a href={`/sessions/${activity.id}`} class="block m-[10px] p-[10px] hover:scale-105 transition 
            ...
            <span class="text-[20px] sm:text-[24px] active:scale-110">{activity.activity_name}</span>
            <hr>
