//#region node_modules/@neodrag/svelte/dist/index.js
var DEFAULT_RECOMPUTE_BOUNDS = { dragStart: true };
var DEFAULT_DRAG_THRESHOLD = {
	delay: 0,
	distance: 3
};
function draggable(node, options = {}) {
	let { bounds, axis = "both", gpuAcceleration = true, legacyTranslate = false, transform, applyUserSelectHack = true, disabled = false, ignoreMultitouch = false, recomputeBounds = DEFAULT_RECOMPUTE_BOUNDS, grid, threshold = DEFAULT_DRAG_THRESHOLD, position, cancel, handle, defaultClass = "neodrag", defaultClassDragging = "neodrag-dragging", defaultClassDragged = "neodrag-dragged", defaultPosition = {
		x: 0,
		y: 0
	...
	function set_translate(x_pos = translate_x, y_pos = translate_y) {
		...
		if (is_string(transform_called)) set_style(node, "transform", transform_called);
	}
	...
	listen("pointerdown", (e) => {
		...
		if (drag_els.some((el) => el.contains(event_target) || el.shadowRoot?.contains(event_target)) && !cancel_element_contains(cancel_els, [event_target])) {
			...
			is_interacting = true;
			start_time = Date.now();
			if (!threshold.delay) meets_time_threshold = true;
		} else return;
	...
	listen("pointermove", (e) => {
		...
		if (!is_dragging) {
			if (!meets_time_threshold) {
				if (Date.now() - start_time >= threshold.delay) {
			...
			if (!meets_distance_threshold) {
				...
				if (Math.sqrt(delta_x ** 2 + delta_y ** 2) >= threshold.distance) {
...
var snap_to_grid = ([x_snap, y_snap], pending_x, pending_y) => {
	...
	return [calc(pending_x, x_snap), calc(pending_y, y_snap)];
};
